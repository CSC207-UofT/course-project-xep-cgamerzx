...
Android Room is also special in that all the functions that would usually be allocated for a gateway are handled by it during the compilation process and autogenerated. There is no need for us to write code ourselves to communicate with the underlying SQLite database.
